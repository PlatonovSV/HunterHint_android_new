...
interface BookingRetrofitService {
    @POST("booking/new")
    suspend fun createBooking(@Header("Authorization") token: String, @Body booking: NewBookingDto): Long

    @GET("booking/all")
    suspend fun getUsersBooking(@Header("Authorization") token: String): List<BookingCardDto>
    @GET("booking/{id}")
    suspend fun getBooking(@Header("Authorization") jwt: String, @Path("id") id: Long): BookingDataDto
}